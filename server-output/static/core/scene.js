...
class Scene {
  ...
  _update_from_editor(update_obj) {
    const target = this._s_index.get(update_obj._id);

    if (!target) {
      console.error(`Object with _id ${update_obj._id} not found in scene`);
      return false;
    }
    Object.assign(target, update_obj);
